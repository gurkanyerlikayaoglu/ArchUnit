...
class JavaClassDependencies {
    ...
    private Set<Dependency> genericSuperclassTypeArgumentDependencies() {
        if (!javaClass.getGenericSuperclass().isPresent() || !(javaClass.getGenericSuperclass().get() instanceof JavaParameterizedType)) {
            return emptySet();
        }
        JavaParameterizedType genericSuperclass = (JavaParameterizedType) javaClass.getGenericSuperclass().get();
